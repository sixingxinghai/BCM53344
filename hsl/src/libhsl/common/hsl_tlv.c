    ...
    if (CHECK_CINDEX (msg->cindex, i))
      {
        ...
        switch (i)
          ...
          case HAL_MSG_CINDEX_IF_AUTONEGO:
            ...
            break;
          case HAL_MSG_CINDEX_IF_IF_FLAGS:
            hsl_msg_encode_tlv (pnt, size, i, 4);
            TLV_ENCODE_PUTL (msg->if_flags);
            break;
...
*/
int
hsl_msg_encode_err_frame_secs_req (u_char **pnt, u_int32_t *size, 
                                   u_int32_t *msg)
{
  unsigned char *sp = *pnt;
...
*/
int
hsl_msg_decode_err_frame_secs_req (u_char **pnt, u_int32_t *size, 
                                   u_int32_t *msg)
{
  unsigned char *sp = *pnt;
    ...
    struct hal_msg_ipv4mc_mrt_add *msg)
{
  ...
  /* Decode olist */
  /* Add 1 to avoid allocating 0 size memory for empty olist */
  msg->olist_ttls = oss_malloc (((msg->num_olist + 1) * sizeof (u_char)), 
      OSS_MEM_HEAP);
    ...
    struct hal_msg_ipv6mc_mrt_add *msg)
{
  ...
  msg->olist = oss_malloc (((msg->num_olist + 1) * sizeof (u_int16_t)), 
      OSS_MEM_HEAP);
  if (msg->olist == NULL)
    return -1;
