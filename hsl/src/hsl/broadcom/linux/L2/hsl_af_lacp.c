...
static struct hlist_head _lacp_socklist;
#else
static struct sock *_lacp_socklist = 0;
#endif
// qcl 20170808 static rwlock_t _lacp_socklist_lock = RW_LOCK_UNLOCKED;
static DEFINE_RWLOCK(_lacp_socklist_lock);
/* Private packet socket structures. */

/* Forward declarations. */
static int _lacp_sock_release (struct socket *sock);
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,27))
static int _lacp_sock_create (struct net *net, struct socket *sock, 
                              int protocol);
#else
static int _lacp_sock_create (struct socket *sock, int protocol);
#endif
...
//qcl 20170808 static struct proto_ops SOCKOPS_WRAPPED (lacp_ops) = {
...
#ifdef LINUX_KERNEL_2_6
static struct proto _lacp_prot = {
        .name     = "HSL_LACP",
        .owner    = THIS_MODULE,
...
#else
_lacp_sock_create (struct socket *sock, int protocol)
#endif
{
  ...
  sock->ops = &lacp_ops;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,27))
  HSL_SOCK_ALLOC(net, sock, AF_LACP, protocol, _lacp);
#else
  HSL_SOCK_ALLOC(sock, AF_LACP, protocol, _lacp);
#endif
  return 0;
}
...
static int 
#ifdef LINUX_KERNEL_2_6
_lacp_sock_sendmsg (struct kiocb *iocb, struct socket *sock, 
...
static int 
#ifdef LINUX_KERNEL_2_6
_lacp_sock_recvmsg (struct kiocb *iocb, struct socket *sock, 
