...
static struct hlist_head _stp_socklist;
#else
...
static int _stp_sock_create (struct net *net, struct socket *sock,
                             int protocol);
#else
static int _stp_sock_create (struct socket *sock, int protocol);
#endif
#ifdef LINUX_KERNEL_2_6
...
#else
static int _stp_sock_sendmsg (struct socket *sock, struct msghdr *msg, int len, struct scm_cookie *scm);
static int _stp_sock_recvmsg (struct socket *sock, struct msghdr *msg, int len, int flags, struct scm_cookie *scm);
#endif
...
#else
_stp_sock_create (struct socket *sock, int protocol)
#endif
{
  ...
  sock->ops = &stp_ops;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,27))
  HSL_SOCK_ALLOC(net, sock, AF_STP, protocol, _stp);
#else
  HSL_SOCK_ALLOC(sock, AF_STP, protocol, _stp);
#endif
  return 0;
}
...
static int 
#ifdef LINUX_KERNEL_2_6
_stp_sock_sendmsg (struct kiocb *iocb, struct socket *sock, 
...
#endif
{
  ...
  if (bridge_port->type == HAL_VLAN_PROVIDER_CN_PORT
      || bridge_port->type == HAL_VLAN_PROVIDER_PN_PORT)
    eth->d.vlan.tag_type = htons (HSL_L2_ETH_P_8021Q_STAG);
  else
    eth->d.vlan.tag_type = htons (HSL_L2_ETH_P_8021Q);
#else
  eth->d.vlan.tag_type = htons (HSL_L2_ETH_P_8021Q);
...
static int 
#ifdef LINUX_KERNEL_2_6
_stp_sock_recvmsg (struct kiocb *iocb, struct socket *sock, 
