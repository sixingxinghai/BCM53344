...
msgUnpackHeader(void *buf, MsgHeader * header)
{
	...
	header->versionPTP = (*(UInteger4 *) (buf + 1)) >> 4;
...
void 
msgPackFollowUp(void *buf, Octet *msgIbuf,MsgHeader * header,Timestamp * preciseOriginTimestamp, PtpClock * ptpClock)
{
	/* changes in header */
	*(char *)(buf + 0) = *(char *)(buf + 0) & 0xF0;
	/* RAZ messageType */
	...
	*(UInteger16 *) (buf + 2) = flip16(FOLLOW_UP_LENGTH);
	/*correction field...multiplied by 2^16. */
	header->correctionfield.lsb = preciseOriginTimestamp->nanosecondsField <<16;
	header->correctionfield.msb = preciseOriginTimestamp->nanosecondsField >>16;
	*(UInteger32 *) (buf + 8) = flip32(header->correctionfield.msb);
	*(UInteger32 *) (buf + 12) = flip32(header->correctionfield.lsb);
	memcpy((buf + 20),header->sourcePortIdentity.clockIdentity,  
	       CLOCK_IDENTITY_LENGTH);
	*(UInteger16 *) (buf + 28) = flip16(header->sourcePortIdentity.portNumber);
	...
	*(UInteger8 *) (buf + 32) = 0x02;
	/* Table 23 */
	*(Integer8 *) (buf + 33) = header->logMessageInterval;

	/* Follow_up message */
	*(UInteger16 *) (buf + 34) = *(UInteger16 *) (msgIbuf + 34);
	*(UInteger32 *) (buf + 36) = *(UInteger32 *) (msgIbuf + 36);
	*(UInteger32 *) (buf + 40) = *(UInteger32 *) (msgIbuf + 40);
	/*
	*(UInteger16 *) (buf + 34) = 
...
msgPackPDelayReq(void *buf, Timestamp * originTimestamp, PtpClock * ptpClock)
{
	...
	*(UInteger16 *) (buf + 4) = 0;//djg. For a not syntonized p2p transparent clock, the domainNumber  shall be set to 0. 
	*(UInteger8 *) (buf + 20) = 0x01;
	*(UInteger8 *) (buf + 28) = 0x0f;
...
msgPackPDelayResp(void *buf, MsgHeader * header, Timestamp * requestReceiptTimestamp, PtpClock * ptpClock)
{
	...
	*(UInteger8 *) (buf + 4) = header->domainNumber;
	/*two step flag*/
	*(char *)(buf + 6) = *(char *)(buf + 6) | 0x02;
	memset((buf + 8), 0, 8);

    *(char *)(buf + 20) = 0x00;*(char *)(buf + 21) = 0x11; *(char *)(buf + 22) = 0x80;*(char *)(buf + 23) = 0xff;
	*(char *)(buf + 24) = 0xfe;*(char *)(buf + 25) = 0x00; *(char *)(buf + 26) = 0x52;*(char *)(buf + 27) = 0xac;
	*(char *)(buf + 28) = 0x00;*(char *)(buf + 29) = 0x01;
	...
	*(UInteger16 *) (buf + 52) = flip16(header->sourcePortIdentity.portNumber);
	//HSL_LOG (HSL_LOG_PKTDRV, HSL_LEVEL_INFO, "ptp test:portID is %d!3\n",header->sourcePortIdentity.portNumber);
	//HSL_LOG (HSL_LOG_PKTDRV, HSL_LEVEL_INFO, "ptp test:portID is %c!4\n", *(UInteger8 *)(buf+52) );
...
msgPackPDelayRespFollowUp(void *buf, MsgHeader * header, Timestamp * responseOriginTimestamp, PtpClock * ptpClock)
{
	...
	*(UInteger8 *) (buf + 4) = header->domainNumber;//djg
	//*(UInteger16 *) (buf + 30) = flip16(ptpClock->PdelayReqHeader.sequenceId);
	*(UInteger16 *) (buf + 30) = flip16(header->sequenceId);//djg
	*(UInteger8 *) (buf + 32) = 0x05;
	/* Table 23 */
	*(Integer8 *) (buf + 33) = 0x7F;
	/* Table 24 */
