...
typedef struct {
	...
	Integer8 phy_port;//0-23
	Boolean enable;//enable virtual port, FALSE = 0, TURE != 0
	Boolean isMasterPort;
	Boolean switchFlag;//如果port 0没有1588报文,则切换,发送端口1的报文
...
{
    int is_used;
    TimeInternal  ingress_sync_time;
	TimeInternal  ingress_sync_time_n;
	TimeInternal  correct_sync_time;
	TimeInternal  correct_sync_time_n;
	int differ;//((ingress_sync_time.nanoseconds + (ingress_sync_time.seconds- ingress_sync_time_n.seconds) * 1000000000 - ingress_sync_time_n.nanoseconds) - (correct_sync_time.nanoseconds + (correct_sync_time.seconds- correct_sync_time_n.seconds) * 1000000000 - correct_sync_time_n.nanoseconds))/10000
	int div;//(correct_sync_time.nanoseconds + (correct_sync_time.seconds- correct_sync_time_n.seconds) * 1000000000 - correct_sync_time_n.nanoseconds)/10000;  
	int flag;
	//int fk;//fk = (ingress_sync_time_n - ingress_sync_time)*100000000 / (correct_sync_time_n - correct_sync_time)
}AdjFreqOpts;
