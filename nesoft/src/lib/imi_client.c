...
#ifdef HAVE_CUSTOM1
int
imi_client_start_timer (struct thread *thread)
{
  ...
  fp = pal_fopen (NSM_DONE_FILE, PAL_OPEN_RO);
  if (fp)
    {
      pal_fclose (fp);
      imi_client_start (ic);
    ...
    thread_add_timer (ic->line.zg, imi_client_start_timer, ic, 1);
                    ...
                    struct message_entry *me, pal_sock_handle_t sock)
{
  ...
  line->pid = 0;
  line->vr_id = 0;
  line->vr  = apn_vr_get_privileged(mc->zg);
                       ...
                       struct message_entry *me, pal_sock_handle_t sock)
{
  ...
  ic->line.vr_id = 0;
  ic->line.vr = apn_vr_get_privileged(mc->zg);
...
/*  printf("%d->%s\n", ic->line.code, ic->line.str); */
  ...
  /* Check VR.  */
  ic->line.vr = apn_vr_lookup_by_id (mc->zg, ic->line.vr_id);
...
*/
#ifdef HAVE_HA
    /* CHECKPOINT: discarding config command on STANDBY */
      if (ic->line.mode >= CONFIG_MODE )
      {
        if (HA_DROP_CFG_ISSET(mc->zg))
        {
          ic->line.code = LINE_CODE_SUCCESS;
          ret = imi_client_send_reply (ic);
          break;
        }
      } /* END OF CHECKPOINT intrusion... */
#endif /* HAVE_HA */
      cmd_str = ic->line.str;
...
#ifdef HAVE_IMISH
          imi_client_set_error(ic,"%% Interfering with IMISH session"
                               " - pid:%d\n",sid);
#else
...
#ifdef HAVE_HA
      /* CHECKPOINT: Reporting CONFIG_DONE on ACTIVE and STANDBY */
      HA_CONFIG_DONE_CB_EXEC(mc->zg);
      /* END_OF_CHECKPOINT intrusion */
#endif /* HAVE_HA */
...
imi_client_create (struct lib_globals *zg, int now)
{
  ...
  /* Start the IMI client.  */
#ifdef HAVE_CUSTOM1
  if (now || zg->protocol == APN_PROTO_NSM)
    imi_client_start (ic);
  else
    thread_add_timer (zg, imi_client_start_timer, ic, 1);
#else /* HAVE_CUSTOM1 */
  imi_client_start (ic);
#endif /* HAVE_CUSTOM1 */
...
int
imi_client_send_config_request (struct apn_vr *vr)
{
