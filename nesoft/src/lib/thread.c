...
/*
   ...
   PacOS as strict "BACKGROUND".  The BACKGROUND MANAGER Keeps track
   of all PacOS Routers, and calls them in strict sequence.  When
                 ...
                 void *arg, long timer)
{
  ...
  pal_timeofday(&thread->stime, NULL);
#endif /* HAVE_HA */
...
thread_fetch (struct lib_globals *zg, struct thread *fetch)
{
  ...
  zg->vr_in_cxt = apn_vr_get_privileged(zg);
