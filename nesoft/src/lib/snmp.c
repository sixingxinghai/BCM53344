...
snmp_progname_set ()
{
  ...
  zsnprintf (snmp_progname, 100, "pacos-%s", pacos_version (buf, 50));
}
...
#ifdef HAVE_IMI
void
imi_snmp_debug_cli_init (struct lib_globals *zg)
{
  struct cli_tree *ctree = zg->ctree;
  ...
  /* snmp debug flags */
  cli_install_imi (ctree, EXEC_MODE, PM_SNMP_DBG, PRIVILEGE_MAX, 0,
                   &debug_snmp_cli);
  cli_install_imi (ctree, EXEC_MODE, PM_SNMP_DBG, PRIVILEGE_MAX, 0,
                   &no_debug_snmp_cli);
#ifdef HAVE_AGENTX
