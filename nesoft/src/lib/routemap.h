...
#ifndef _PACOS_ROUTEMAP_H
#define _PACOS_ROUTEMAP_H
...
struct snmpCommunity
{
  ...
  void (*event_hook) (struct apn_vr *, char *);
};
...
struct route_map_list
{
  ...
  void (*add_hook) (struct apn_vr *, char *);
  void (*delete_hook) (struct apn_vr *, char *);
  void (*event_hook) (struct apn_vr *, route_map_event_t, char *);
};
...
void route_map_install_set (struct apn_vr *, struct route_map_rule_cmd *);
void route_map_install_match (struct apn_vr *, struct route_map_rule_cmd *);
void route_map_add_hook (struct apn_vr *,
                         void (*func) (struct apn_vr *, char *));
void route_map_delete_hook (struct apn_vr *,
                            void (*func) (struct apn_vr *, char *));
void route_map_event_hook (struct apn_vr *,
                           void (*func) (struct apn_vr *, route_map_event_t,
                                         char *));

route_map_result_t route_map_apply (struct route_map *,
                                    struct prefix *, void *);
struct route_map *route_map_lookup_by_name (struct apn_vr *, char *);

struct route_map_index *route_map_index_install (struct apn_vr *, char *, int,
                                                 int);
int route_map_uninstall (struct apn_vr *, char *);
int route_map_index_uninstall (struct apn_vr *, char *, int, int);
void route_map_index_delete (struct apn_vr *, struct route_map_index *, int);

int route_map_match_set (struct apn_vr *, char *, int, int, char *, char *);
int route_map_match_unset (struct apn_vr *, char *, int, int, char *, char *);
int route_map_set_set (struct apn_vr *, char *, int, int, char *, char *, int);
int route_map_set_unset (struct apn_vr *, char *, int, int, char *, char *);

void route_map_init (struct apn_vr *);
void route_map_finish (struct apn_vr *);
void route_map_cli_init (struct lib_globals *);
int route_map_config_write (struct cli *);
int route_map_config_encode (struct apn_vr *vr, cfg_vect_t *cv);
struct route_map_rule_cmd * route_map_lookup_match (struct apn_vr *vr, 
                                                    char *name);
int route_map_set_set_nexthop (struct apn_vr *vr, char *name, int direct, 
                               int pref, char *command, char *nexthop, 
                               s_int16_t nh_type, char *ifname, int flags);
int route_map_set_unset_nexthop (struct apn_vr *vr, char *name, int direct,
                                 int pref, char *command, char *nexthop,
                                 s_int16_t nh_type, char *ifname);
struct route_map_rule_cmd * route_map_lookup_set (struct apn_vr *vr, char *name);

#endif /* _PACOS_ROUTEMAP_H */
