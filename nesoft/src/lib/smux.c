                   ...
                   long errindex, struct list *varlist)
{
  ...
  ret = pal_sock_send (zg->snmp.sock, buf, (ptr - buf), 0);
}

char *
smux_var (struct lib_globals *zg, char *ptr, s_int32_t *len, oid objid[],
...
/* SMUX_RRSP message. */
void
smux_parse_rrsp (struct lib_globals *zg, char *ptr, s_int32_t len)
{
  char val;
  long errstat;
...
/* Parse SMUX message. */
s_int32_t
smux_parse (struct lib_globals *zg, char *ptr, s_int32_t len)
{
