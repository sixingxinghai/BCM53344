...
#ifndef _PACOS_HOST_H
#define _PACOS_HOST_H
...
typedef int (*HOST_CALLBACK) (struct apn_vr *);
...
#define HOST_CONFIG_START(Z, F, P)                                            \
    do {                                                                      \
      if ((Z)->protocol != APN_PROTO_IMI)                                     \
        {                                                                     \
           apn_vr_add_callback ((Z), VR_CALLBACK_CONFIG_READ,                 \
...
#define HOST_CONFIG_VR_START(V)                                               \
    do {                                                                      \
        if ((V)->zg->protocol != APN_PROTO_IMI)                               \
          {                                                                   \
            apn_vr_add_callback ((V)->zg, VR_CALLBACK_CONFIG_READ,            \
                imi_client_send_config_request);                              \
                                                                              \
...
#define HOST_CONFIG_START(Z, F, P)                                            \
    do {                                                                      \
      struct apn_vr *_vr = apn_vr_get_privileged (Z);                         \
                                                                              \
      host_startup_config_file_set (_vr, F);                                  \
                                                                              \
      vty_serv_sock (Z, P);                                                   \
                                                                              \
      apn_vr_add_callback ((Z), VR_CALLBACK_CONFIG_READ, host_config_read);   \
                                                                              \
...

/* Prototypes.  */
struct host *host_new (struct apn_vr *);
void host_free (struct host *);

int host_config_lock (struct host *, void *);
int host_config_unlock (struct host *, void *);

void host_config_file_set (struct apn_vr *);
void host_startup_config_file_set (struct apn_vr *, char *);
int host_config_read (struct apn_vr *);
...
int host_hostname_set (struct apn_vr *, char *);
int host_hostname_unset (struct apn_vr *, char *);
int host_hostname_set_callback (struct apn_vr *, HOST_CALLBACK);
