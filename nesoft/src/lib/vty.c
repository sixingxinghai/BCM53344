...
void
vty_serv_add_vty_gone_cb(struct lib_globals *zg,
                         void (*func)(struct vty *))
{
  zg->vty_master->vty_serv_vty_gone_cb = func;
}
#endif
...
#if defined(HAVE_IMI) && !defined(HAVE_IMISH)
      /* IMI intercepts command for protocol module proxy. */
      ret = (*server->imi_vty_command) (vty, node, buf);
#else
...
struct vty *
vty_create (struct lib_globals *zg, struct apn_vr *vr,
...
vty_accept_access_list_apply (struct vty_server *server,
                              struct apn_vr *vr, struct prefix *p)
{
...
int
vty_vr_unbind (struct apn_vr *vr)
{
  int i;
  struct vty *vty;
  struct vty_server *server = vr->zg->vty_master;
...
vty_serv_sock (struct lib_globals *zg, u_int16_t port)
{
  ...
  apn_vr_add_callback (zg, VR_CALLBACK_CLOSE, vty_vr_close);

  /* Set VR unbind callback function.  */
  apn_vr_add_callback (zg, VR_CALLBACK_UNBIND, vty_vr_unbind);
...
vty_close (struct vty *vty)
{
  ...
  switch (vty->type)
    ...
    case VTY_SHELL_SERV:
      if (vty->sock >= 0) {
#if defined (HAVE_IMI) && ! defined (HAVE_IMISH)
        /* Notify registered user about VTY departure.
           This is used e.g. by IMI to flush config sessions.
        */
        if (server->vty_serv_vty_gone_cb!=NULL)
            server->vty_serv_vty_gone_cb(vty);
#endif
...
/* Set time out value. */
int
exec_timeout (struct apn_vr *vr, struct vty *vty, char *min_str, char *sec_str)
{
     ...
     CLI_VR_NAME_STR)
{
  ...
  vr = apn_vr_lookup_by_name (cli->vr->zg, argv[0]);
  if (vr == NULL)
    {
