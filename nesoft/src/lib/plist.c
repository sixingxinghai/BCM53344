...
prefix_master_get (struct apn_vr *vr, afi_t afi)
{
  if (afi == AFI_IP)
...
struct prefix_list *
prefix_list_lookup (struct apn_vr *vr, afi_t afi, char *name)
{
...
struct prefix_list *
prefix_list_insert (struct apn_vr *vr, afi_t afi, char *name)
{
  int i;
  long number;
  struct prefix_list *plist;
...
struct prefix_list *
prefix_list_get (struct apn_vr *vr, afi_t afi, char *name)
{
  struct prefix_list *plist;
...
/* Add hook function. */
void
prefix_list_add_hook (struct apn_vr *vr, void (*func) ())
{
...
void
prefix_list_delete_hook (struct apn_vr *vr, void (*func) ())
{
...
int
prefix_list_install (struct apn_vr *vr, afi_t afi,
...
int
prefix_list_uninstall (struct apn_vr *vr, afi_t afi,
                       ...
                       int seqnum)
{
  ...
  if (cli->zg->protocol != APN_PROTO_IMI || !disp_flag)
    {
#endif /* HAVE_IMI */
  ...
  else
    {
      if (dtype == detail_display || dtype == summary_display)
        {
          if (cli->zg->protocol == APN_PROTO_IMI)
            if (master->recent)
              cli_out (cli, "Prefix-list with the last deletion/insertion: %s\n",
                       master->recent->name);
        }
      ...
      do {
        for (; plist; plist = plist->next)
          {
#ifdef HAVE_IMI
            mod_str = XCALLOC (MTYPE_TMP, pal_strlen (cli->str) +
                                          pal_strlen (plist->name) + 2);

            if (! mod_str)
              continue;

            pal_strcpy (mod_str, cli->str);
            pal_strcat (mod_str, " ");
            pal_strcat (mod_str, plist->name);
...
int
config_encode_prefix_afi (struct apn_vr *vr, afi_t afi, cfg_vect_t *cv)
{
  struct prefix_list *plist;
  struct prefix_list_entry *pentry;
  struct prefix_master *master;
...
int
prefix_bgp_orf_set (struct apn_vr *vr, char *name, afi_t afi,
...
void
prefix_bgp_orf_remove_all (struct apn_vr *vr, char *name)
{
  struct prefix_list *plist;
...
/* return prefix count */
int
prefix_bgp_show_prefix_list (struct apn_vr *vr, struct cli *cli,
                             afi_t afi, char *name)
{
  struct prefix_list *plist;
  struct prefix_list_entry *pentry;
...
void
prefix_list_finish_orf (struct apn_vr *vr)
{
  struct prefix_list *plist;
...
int
config_encode_prefix_ipv4 (struct apn_vr *vr, cfg_vect_t *cv)
{
...
void
prefix_list_finish_ipv4 (struct apn_vr *vr)
{
  struct prefix_list *plist;
  struct prefix_list *next;
  struct prefix_master *master;
...
#ifdef HAVE_IPV6
int
config_encode_prefix_ipv6 (struct apn_vr *vr, cfg_vect_t *cv)
{
...
void
prefix_list_finish_ipv6 (struct apn_vr *vr)
{
  struct prefix_list *plist;
  struct prefix_list *next;
  struct prefix_master *master;
...
void
prefix_list_finish (struct apn_vr *vr)
{
...
#ifdef HAVE_IMI
void
imi_prefix_list_init (struct lib_globals *zg)
{
  struct cli_tree *ctree = zg->ctree;

  cli_install_imi (ctree, CONFIG_MODE, PM_PREFIX, PRIVILEGE_NORMAL, 0,
                   &ip_prefix_list_cli);
  cli_set_imi_cmd (&ip_prefix_list_cli, CONFIG_MODE, CFG_DTYP_PREFIX_IPV4);

  cli_install_imi (ctree, CONFIG_MODE, PM_PREFIX, PRIVILEGE_NORMAL, 0,
                   &ip_prefix_list_ge_cli);
  cli_set_imi_cmd (&ip_prefix_list_ge_cli, CONFIG_MODE, CFG_DTYP_PREFIX_IPV4);

  cli_install_imi (ctree, CONFIG_MODE, PM_PREFIX, PRIVILEGE_NORMAL, 0,
                   &ip_prefix_list_ge_le_cli);
  cli_set_imi_cmd (&ip_prefix_list_ge_le_cli, CONFIG_MODE, CFG_DTYP_PREFIX_IPV4);

  cli_install_imi (ctree, CONFIG_MODE, PM_PREFIX, PRIVILEGE_NORMAL, 0,
                   &ip_prefix_list_le_cli);
  cli_set_imi_cmd (&ip_prefix_list_le_cli, CONFIG_MODE, CFG_DTYP_PREFIX_IPV4);

  cli_install_imi (ctree, CONFIG_MODE, PM_PREFIX, PRIVILEGE_NORMAL, 0,
                   &ip_prefix_list_le_ge_cli);
  cli_set_imi_cmd (&ip_prefix_list_le_ge_cli, CONFIG_MODE, CFG_DTYP_PREFIX_IPV4);

  cli_install_imi (ctree, CONFIG_MODE, PM_PREFIX, PRIVILEGE_NORMAL, 0,
                   &ip_prefix_list_seq_cli);
  cli_set_imi_cmd (&ip_prefix_list_seq_cli, CONFIG_MODE, CFG_DTYP_PREFIX_IPV4);

  cli_install_imi (ctree, CONFIG_MODE, PM_PREFIX, PRIVILEGE_NORMAL, 0,
                   &ip_prefix_list_seq_ge_cli);
  cli_set_imi_cmd (&ip_prefix_list_seq_ge_cli, CONFIG_MODE, CFG_DTYP_PREFIX_IPV4);

  cli_install_imi (ctree, CONFIG_MODE, PM_PREFIX, PRIVILEGE_NORMAL, 0,
                   &ip_prefix_list_seq_ge_le_cli);
  cli_set_imi_cmd (&ip_prefix_list_seq_ge_le_cli, CONFIG_MODE, CFG_DTYP_PREFIX_IPV4);

  cli_install_imi (ctree, CONFIG_MODE, PM_PREFIX, PRIVILEGE_NORMAL, 0,
                   &ip_prefix_list_seq_le_cli);
  cli_set_imi_cmd (&ip_prefix_list_seq_le_cli, CONFIG_MODE, CFG_DTYP_PREFIX_IPV4);

  cli_install_imi (ctree, CONFIG_MODE, PM_PREFIX, PRIVILEGE_NORMAL, 0,
                   &ip_prefix_list_seq_le_ge_cli);
  cli_set_imi_cmd (&ip_prefix_list_seq_le_ge_cli, CONFIG_MODE, CFG_DTYP_PREFIX_IPV4);
  ...
  cli_install_imi (ctree, CONFIG_MODE, PM_PREFIX, PRIVILEGE_NORMAL, 0,
                   &ip_prefix_list_description_cli);
  cli_set_imi_cmd (&ip_prefix_list_description_cli, CONFIG_MODE, CFG_DTYP_PREFIX_IPV4);
  ...
  cli_install_imi (ctree, CONFIG_MODE, PM_PREFIX, PRIVILEGE_NORMAL, 0,
                   &ip_prefix_list_sequence_number_cli);
  cli_set_imi_cmd (&ip_prefix_list_sequence_number_cli, CONFIG_MODE, CFG_DTYP_PREFIX_IPV4);

  cli_install_imi (ctree, CONFIG_MODE, PM_PREFIX, PRIVILEGE_NORMAL, 0,
                   &no_ip_prefix_list_sequence_number_cli);
  cli_set_imi_cmd (&no_ip_prefix_list_sequence_number_cli, CONFIG_MODE, CFG_DTYP_PREFIX_IPV4);
  ...
  cli_install_imi (ctree, EXEC_MODE, PM_PREFIX, PRIVILEGE_VR_MAX, 0,
                   &clear_ip_prefix_list_cli);
  cli_install_imi (ctree, EXEC_MODE, PM_PREFIX, PRIVILEGE_VR_MAX, 0,
                   &clear_ip_prefix_list_name_cli);
  cli_install_imi (ctree, EXEC_MODE, PM_PREFIX, PRIVILEGE_VR_MAX, 0,
  ...
  cli_install_imi (ctree, CONFIG_MODE, PM_PREFIX, PRIVILEGE_NORMAL, 0,
                   &ipv6_prefix_list_cli);
  cli_set_imi_cmd (&ipv6_prefix_list_cli, CONFIG_MODE, CFG_DTYP_PREFIX_IPV6);

  cli_install_imi (ctree, CONFIG_MODE, PM_PREFIX, PRIVILEGE_NORMAL, 0,
                   &ipv6_prefix_list_ge_cli);
  cli_set_imi_cmd (&ipv6_prefix_list_ge_cli, CONFIG_MODE, CFG_DTYP_PREFIX_IPV6);

  cli_install_imi (ctree, CONFIG_MODE, PM_PREFIX, PRIVILEGE_NORMAL, 0,
                   &ipv6_prefix_list_ge_le_cli);
  cli_set_imi_cmd (&ipv6_prefix_list_ge_le_cli, CONFIG_MODE, CFG_DTYP_PREFIX_IPV6);

  cli_install_imi (ctree, CONFIG_MODE, PM_PREFIX, PRIVILEGE_NORMAL, 0,
                   &ipv6_prefix_list_le_cli);
  cli_set_imi_cmd (&ipv6_prefix_list_le_cli, CONFIG_MODE, CFG_DTYP_PREFIX_IPV6);

  cli_install_imi (ctree, CONFIG_MODE, PM_PREFIX, PRIVILEGE_NORMAL, 0,
                   &ipv6_prefix_list_le_ge_cli);
  cli_set_imi_cmd (&ipv6_prefix_list_le_ge_cli, CONFIG_MODE, CFG_DTYP_PREFIX_IPV6);

  cli_install_imi (ctree, CONFIG_MODE, PM_PREFIX, PRIVILEGE_NORMAL, 0,
                   &ipv6_prefix_list_seq_cli);
  cli_set_imi_cmd (&ipv6_prefix_list_seq_cli, CONFIG_MODE, CFG_DTYP_PREFIX_IPV6);

  cli_install_imi (ctree, CONFIG_MODE, PM_PREFIX, PRIVILEGE_NORMAL, 0,
                   &ipv6_prefix_list_seq_ge_cli);
  cli_set_imi_cmd (&ipv6_prefix_list_seq_ge_cli, CONFIG_MODE, CFG_DTYP_PREFIX_IPV6);

  cli_install_imi (ctree, CONFIG_MODE, PM_PREFIX, PRIVILEGE_NORMAL, 0,
                   &ipv6_prefix_list_seq_ge_le_cli);
  cli_set_imi_cmd (&ipv6_prefix_list_seq_ge_le_cli, CONFIG_MODE, CFG_DTYP_PREFIX_IPV6);

  cli_install_imi (ctree, CONFIG_MODE, PM_PREFIX, PRIVILEGE_NORMAL, 0,
                   &ipv6_prefix_list_seq_le_cli);
  cli_set_imi_cmd (&ipv6_prefix_list_seq_le_cli, CONFIG_MODE, CFG_DTYP_PREFIX_IPV6);

  cli_install_imi (ctree, CONFIG_MODE, PM_PREFIX, PRIVILEGE_NORMAL, 0,
                   &ipv6_prefix_list_seq_le_ge_cli);
  cli_set_imi_cmd (&ipv6_prefix_list_seq_le_ge_cli, CONFIG_MODE, CFG_DTYP_PREFIX_IPV6);
  ...
  cli_install_imi (ctree, CONFIG_MODE, PM_PREFIX, PRIVILEGE_NORMAL, 0,
                   &ipv6_prefix_list_description_cli);
  cli_set_imi_cmd (&ipv6_prefix_list_description_cli, CONFIG_MODE, CFG_DTYP_PREFIX_IPV6);
  ...
  cli_install_imi (ctree, CONFIG_MODE, PM_PREFIX, PRIVILEGE_NORMAL, 0,
                   &ipv6_prefix_list_sequence_number_cli);
  cli_set_imi_cmd (&ipv6_prefix_list_sequence_number_cli, CONFIG_MODE, CFG_DTYP_PREFIX_IPV6);

  cli_install_imi (ctree, CONFIG_MODE, PM_PREFIX, PRIVILEGE_NORMAL, 0,
                   &no_ipv6_prefix_list_sequence_number_cli);
  cli_set_imi_cmd (&no_ipv6_prefix_list_sequence_number_cli, CONFIG_MODE, CFG_DTYP_PREFIX_IPV6);
