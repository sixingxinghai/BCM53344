...
memmgr_get_protocol_id (char *pstr)
{
  if (!pal_strncmp ("nsm", pstr, 1))
    return APN_PROTO_NSM;
  else if (!pal_strncmp ("lib", pstr, 2))
    return APN_PROTO_UNSPEC;
  else if (!pal_strncmp ("imi", pstr, 2))
    return APN_PROTO_IMI;
  else if (!pal_strncmp ("rip", pstr, 2))
    return APN_PROTO_RIP;
  else if (!pal_strncmp ("ipv6 rip", pstr, 6))
    return APN_PROTO_RIPNG;
  else if (!pal_strncmp ("ospf", pstr, 1))
    return APN_PROTO_OSPF;
  else if (!pal_strncmp ("ipv6 ospf", pstr, 6))
    return APN_PROTO_OSPF6;
  else if (!pal_strncmp ("isis", pstr, 2))
    return APN_PROTO_ISIS;
  else if (!pal_strncmp ("bgp", pstr, 1))
    return APN_PROTO_BGP;
  else if (!pal_strncmp ("ldp", pstr, 2))
    return APN_PROTO_LDP;
  else if (!pal_strncmp ("rsvp", pstr, 3))
    return APN_PROTO_RSVP;
  else if (!pal_strncmp ("pim dense-mode", pstr, 5))
    return APN_PROTO_PIMDM;
  else if (!pal_strncmp ("pim sparse-mode", pstr, 5))
    return APN_PROTO_PIMSM;
  else if (!pal_strncmp ("ipv6 pim sparse-mode", pstr, 10))
    return APN_PROTO_PIMSM6;
  else if (!pal_strncmp ("dvmrp", pstr, 2))
    return APN_PROTO_DVMRP;
  else if (!pal_strncmp ("dot1x", pstr, 2)
           || !pal_strncmp ("auth", pstr, 1))
    return APN_PROTO_8021X;
  else if (!pal_strncmp ("onmd", pstr, 2))
    return APN_PROTO_ONM;
  else if (!pal_strncmp ("lacp", pstr, 2))
    return APN_PROTO_LACP;
  else if (!pal_strncmp ("stp", pstr, 1))
    return APN_PROTO_STP;
  else if (!pal_strncmp ("rstp", pstr, 3))
    return APN_PROTO_RSTP;
  else if (!pal_strncmp ("mstp", pstr, 2))
    return APN_PROTO_MSTP;
  else if (!pal_strncmp ("elmi", pstr, 2))
    return APN_PROTO_ELMI;
  else
    return -1;
}
...
memmgr_show_memory_summary (struct cli *cli)
{
     ...
     cli_out (cli, "  Requested PacOS memory size:                   %d\n", req_size);
     cli_out (cli, "  Allocated PacOS memory size:                   %d\n", size);
     cli_out (cli, "  Allocated PacOS memory blocks:                 %d\n", count);
     cli_out (cli, "\n");
