 ...
 *  Global declaration
 */
static struct apn_mem_table  mtype_table[MTYPE_MAX];
static struct apn_mem_table  free_table[BUKT_COUNT];
static struct apn_mem_info   mem_stats;
static struct apn_mem_global mem_global;
...
void   memmgr_partition_memory (int, int, struct apn_mem_table *);
...
void   memmgr_add_to_mtype_table (int, struct apn_memblock_header *, int);
void   memmgr_remove_from_mtype_table (int, void *);
void   memmgr_add_to_free_table (void *);
struct apn_memblock_header * memmgr_get_from_free_table (int);
struct apn_memblock_header * memmgr_get_from_system_heap (int);
int    memmgr_get_apn_overhead_size ();
int    memmgr_get_apn_header_size ();
struct apn_memblock_trailer * memmgr_get_apn_trailer_offset ();
...
memmgr_init_memtable (void)
{
    ...
    /* init memory table */
    pal_mem_set (mtype_table,  0,  sizeof (struct apn_mem_table) * MTYPE_MAX);
    pal_mem_set (free_table,   0,  sizeof (struct apn_mem_table) * BUKT_COUNT);
    ...
    pal_mem_set (&mem_global,   0,  sizeof (struct apn_mem_global));
...
memmgr_free_memtable ()
{
    ...
    int    count;
    int    rt_count;
    struct apn_memblock_trailer *p;
    struct apn_memblock_header  *hdr;
    struct apn_memblock_header  *temp;
    ...
    for (index = 0; index < BUKT_COUNT; index++)
      {
        hdr = (struct apn_memblock_header *) free_table[index].list;
    ...
    for (index = 0; index < MTYPE_MAX; index++)
      {
        hdr = (struct apn_memblock_header *) mtype_table[index].list;
        count = mtype_table[index].count;

        while (count > 0 && hdr != NULL)
          {
            temp = hdr;
            p =  memmgr_get_apn_trailer_offset (hdr);
            hdr = hdr->next;
            if (CHECK_FLAG (temp->flags, MEM_BLK_MALLOC))
              {
                rt_count++;
...
{
    struct apn_memblock_header *mhdr;
    void   *buf;
    int    old_size;

    if (ptr != NULL)
      {
        mhdr = (struct apn_memblock_header *)
               ((char *) ptr - memmgr_get_apn_header_size ());
...
{
    struct apn_memblock_header  *mhdr;
    struct apn_memblock_trailer *mtlr;
    int    i;
    ...
    mtlr = memmgr_get_apn_trailer_offset (mhdr);
 ...
 */
void *
memmgr_get_memory (int count, int block_size, struct apn_mem_table *tbl)
{
    ...
    total_size += count * memmgr_get_apn_overhead_size ();
 ...
 */
void
memmgr_partition_memory (int count,  int block_size, struct apn_mem_table *tbl)
{
     int    apn_size;
     int    hdr_size;
     int    i;
     char   *ptr;
     struct apn_memblock_header *mhdr;      /* 1st memory header in the list */
     struct apn_memblock_header *nmhdr;     /* next memory header */

     if (count == 0)
       return;

     apn_size = block_size +  memmgr_get_apn_overhead_size ();
     hdr_size = memmgr_get_apn_header_size ();

     ptr  = (char *) tbl->list;
     mhdr = (struct apn_memblock_header *) ptr;
     ...
     for (i = 1; i < count; i++)
       {
         ...
         nmhdr = (struct apn_memblock_header *) ptr;
 ...
 */
void
memmgr_add_to_mtype_table (int mtype, struct apn_memblock_header *nmhdr, int req_size)
{
    struct apn_memblock_header *mhdr;
    ...
    if (mtype_table[mtype].list == NULL)
      ...
      {
        mhdr = (struct apn_memblock_header *) mtype_table[mtype].list;
...
memmgr_remove_from_mtype_table (int mtype, void *ptr)
{
    ...
    if (mtype_table[mtype].list == NULL)
      {
        MEMMGR_LOG_ERR ("Can't delete - mtype list is empty\n");
        return;
      }
    ...
    else if (mhdr == (struct apn_memblock_header *) mtype_table[mtype].list)
...
memmgr_add_to_free_table (void *ptr)
{
    ...
    hdr = (struct apn_memblock_header *)
          ((char *) ptr - memmgr_get_apn_header_size ());
    ...
    mhdr = (struct apn_memblock_header *) free_table[index].list;
    hdr->prev  = mhdr->prev;
    hdr->next  = mhdr;
    hdr->next->prev = hdr;
    hdr->prev->next = hdr;
...
memmgr_get_from_free_table (int index)
{
    ...
    apn_size = memmgr_get_bucket_block_size (index) + memmgr_get_apn_overhead_size ();
    ...
    /* update bucket stats */
    free_table[index].count--;
    free_table[index].size -= apn_size;
...
memmgr_get_from_system_heap (int req_size)
{
     ...
     mhdr = (struct apn_memblock_header *) malloc (apn_size);
     if (mhdr == NULL)
       return mhdr;
     ...
     pal_mem_set (mhdr, 0, sizeof (struct apn_memblock_header));
 ...
 */
void
memmgr_set_memblock_trailer (struct apn_memblock_header *hdr, int index,
...
#ifdef MEMMGR_RUNTIME_CHECK
  pal_mem_set (&mtlr, 0, sizeof (struct apn_memblock_trailer));
    ...
    ptr = (char *) hdr;
    ptr += memmgr_get_apn_header_size () + memmgr_get_bucket_block_size (index);

    pal_mem_cpy (ptr, &mtlr, sizeof (struct apn_memblock_trailer));
...
memmgr_get_user_membuf (int req_size, int mtype, char *filename, int line)
{
    ...
    ptr = (char *) mhdr + memmgr_get_apn_header_size ();
...
memmgr_set_pa_stats (int count, int block_size, void * ptr)
{
    ...
    mem_stats.pa_mem_overhead += count * memmgr_get_apn_overhead_size ();
...
memmgr_set_rt_stats (int block_size)
{
    ...
    mem_stats.rt_mem_overhead += memmgr_get_apn_overhead_size ();
}
...
memmgr_get_apn_overhead_size ()
{
    ...
    size = sizeof (struct apn_memblock_header);

#ifdef MEMMGR_RUNTIME_CHECK
    size += sizeof (struct apn_memblock_trailer);
#endif
...
memmgr_get_apn_header_size ()
{
    return sizeof (struct apn_memblock_header);
}

/*
 *  memmgr_get_apn_trailer_offset ()
 *
 *  return the ipi memory block trailer size.
 */
struct apn_memblock_trailer *
memmgr_get_apn_trailer_offset (struct apn_memblock_header *hdr)
{
    int  size;
    char *ptr;
    ...
    ptr = (char *) hdr + (memmgr_get_apn_header_size () + size);

    return (struct apn_memblock_trailer *) ptr;
}
...
memmgr_sem_create (void)
{
    ...
    semId = pal_sem_create (APN_SEM_TYPE_MUTEX);
...
memmgr_sem_take (void)
{
    ...
    ret = pal_sem_take (semId, APN_WAIT_FOREVER);
