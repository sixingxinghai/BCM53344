...
host_user_add_callback (struct lib_globals *zg, enum user_callback_type type,
                        int (*func) (struct apn_vr *, struct host_user *))
{
...
void
host_config_file_set (struct apn_vr *vr)
{
  char *path;
  char buf[MAXPATHLEN];
  ...
  if (vr->zg->protocol == APN_PROTO_IMI)
...
void
host_startup_config_file_set (struct apn_vr *vr, char *config_file)
{
  int i;
  FILE *fp;
  char *path, *old_cwd;
...
int
host_config_read (struct apn_vr *vr)
{
  int ret;
  FILE *fp;
  int length;
...
{
  struct apn_vr *vr = THREAD_ARG (thread);
...
int
host_config_write_file (struct cli *cli, struct apn_vr *vr)
{
  int i;
  FILE *fp;
  char buf[BUFSIZ];
...
int
host_hostname_set (struct apn_vr *vr, char *hostname)
{
  struct host *host = vr->host;
...
int
host_hostname_unset (struct apn_vr *vr, char *hostname)
{
  struct host *host = vr->host;
...
int
host_hostname_set_callback (struct apn_vr *vr, HOST_CALLBACK func)
{
  if (vr->host == NULL)
    return 0;
     ...
     "Display PacOS version")
{
  ...
  fp = pal_fopen ("/usr/local/etc/PacOS.crc", PAL_OPEN_RO);
  if (fp != NULL)
  {
      while (pal_fgets (buf, BUFSIZ, fp))
      cli_out (cli, "%s", buf);
...
/* wmx@150128 no password命令改为reset password,功能为把Password设为默认值pacos */
CLI (no_host_password,
     no_host_password_cli,
     "reset password",
     CLI_NO_STR,
     "Specify password 'pacos'")
{
  struct host *host = cli->vr->host;
  char cryptbuf[16];
  ...
  if (CHECK_FLAG (host->flags, HOST_PASSWORD_ENCRYPT))
    host->password_encrypt = XSTRDUP (MTYPE_CONFIG_PASSWORD,
                                      zencrypt ("pacos", cryptbuf));
  else
    host->password = XSTRDUP (MTYPE_CONFIG_PASSWORD, "pacos");
                   ...
                   char *config_file, u_int16_t vty_port)
{
  ...
  HOST_CONFIG_START (zg, config_file, APN_VTY_PORT (zg, vty_port));
#else
  vty_serv_sock (zg, APN_VTY_PORT (zg, vty_port));
#endif /* !HAVE_NO_STORAGE_DEV || HAVE_IMI */

 /* If there is no storage device mark as startup config complete. */
#ifdef HAVE_NO_STORAGE_DEV
  SET_FLAG ((apn_vr_get_privileged (zg))->host->flags, HOST_CONFIG_READ_DONE);
