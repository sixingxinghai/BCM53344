...
#ifdef HAVE_VRF
struct route_map_rule_cmd route_map_vpnv4_nexthop_cmd =
{
  "vpnv4 next-hop",
  NULL,
  NULL,
...
route_map_add (struct apn_vr *vr, char *name)
{
...
static void
route_map_delete (struct apn_vr *vr, struct route_map *map)
{
  struct route_map_list *list;
...
struct route_map *
route_map_lookup_by_name (struct apn_vr *vr, char *name)
{
  struct route_map *map;
...
/* Lookup route map.  If there isn't route map create one and return
   it. */
struct route_map *
route_map_get (struct apn_vr *vr, char *name)
{
  struct route_map *map;
...
void
route_map_install_match_default (struct apn_vr *vr,
...
void
route_map_install_set_default (struct apn_vr *vr,
                               struct route_map_rule_cmd *cmd)
{
...
/* Install rule command to the match list. */
void
route_map_install_match (struct apn_vr *vr, struct route_map_rule_cmd *cmd_add)
{
  struct route_map_rule_cmd *cmd;
  int i;
...
/* Install rule command to the set list. */
void
route_map_install_set (struct apn_vr *vr, struct route_map_rule_cmd *cmd_add)
{
  struct route_map_rule_cmd *cmd;
  int i;
...
route_map_lookup_match (struct apn_vr *vr, char *name)
{
  struct route_map_rule_cmd *cmd;
  int i;
...
struct route_map_rule_cmd *
route_map_lookup_set (struct apn_vr *vr, char *name)
{
  int i;
  struct route_map_rule_cmd *cmd;
...
void
route_map_index_delete (struct apn_vr *vr,
...
#ifdef HAVE_PBR
  /* Execute event */
  if (vr->pbr_event.event_hook)
    (*vr->pbr_event.event_hook)(PBR_RMAP_EVENT_INDEX_DELETE, vr, (void *)index);    
#endif /* HAVE_PBR */
...
struct route_map_index *
route_map_index_add (struct apn_vr *vr, struct route_map *map,
...
/* Get route map index. */
struct route_map_index *
route_map_index_get (struct apn_vr *vr, struct route_map *map,
                     enum route_map_type type, s_int32_t pref)
{
  struct route_map_index *index;
...
struct route_map_index *
route_map_index_install (struct apn_vr *vr,
...
int
route_map_uninstall (struct apn_vr *vr, char *name)
{
  struct route_map *map;
...
int
route_map_index_uninstall (struct apn_vr *vr, char *name,
                           int permit, int pref)
{
  struct route_map_index *index;
  struct route_map *map;
...
void
route_map_add_hook (struct apn_vr *vr, void (*func) (struct apn_vr *, char *))
{
  vr->route_map_master.add_hook = func;
}

void
route_map_delete_hook (struct apn_vr *vr,
                       void (*func) (struct apn_vr *, char *))
{
  vr->route_map_master.delete_hook = func;
}

void
route_map_event_hook (struct apn_vr *vr,
                      void (*func) (struct apn_vr *, route_map_event_t,
                                    char *))
{
...
int
route_map_match_set (struct apn_vr *vr,
                     ...
                     char *command, char *arg)
{
  ...
  int replaced = 0;
#ifdef HAVE_PBR
  struct pbr_rmap_match match_data;
#endif /* HAVE_PBR */
      ...
      if (rule->cmd == cmd)
        {
          ...
          /* Execute event */
          if (vr->pbr_event.event_hook && 
              (pal_strcmp (rule->rule_str, PBR_ROUTE_MAP_MATCH_COMMAND) == 0))
            {
              match_data.map_name = map->name;
              match_data.index = index;
              match_data.rule_str = rule->rule_str;
              match_data.cmd = RMAP_POLICY_DELETE;

              (*vr->pbr_event.event_hook)(PBR_RMAP_EVENT_MATCH_IP_ADDR_ADD_DEL, 
                                          vr, (void *)&match_data);
            }
#endif /* HAVE_PBR */ 
          route_map_rule_delete (&index->match_list, rule);
  ...
  if (vr->pbr_event.event_hook &&
      (pal_strcmp (rule->rule_str, PBR_ROUTE_MAP_MATCH_COMMAND) == 0))
    {
      ...
      match_data.index = index;
      match_data.rule_str = rule->rule_str;
      match_data.cmd = RMAP_POLICY_ADD;
...
int
route_map_match_unset (struct apn_vr *vr,
                       char *name, int direct, int pref,
                       char *command, char *arg)
{
  struct route_map *map;
...
int
route_map_set_set (struct apn_vr *vr,
                   char *name, int direct, int pref,
...
int
route_map_set_unset (struct apn_vr *vr,
                     char *name, int direct, int pref,
                     char *command, char *arg)
{
  struct route_map *map;
...
int route_map_set_set_nexthop (struct apn_vr *vr,
                               char *name, int direct, int pref,
                               ...
                               char *ifname, int flags)
{
  ...
  int replaced = 0;
#ifdef HAVE_PBR
  struct pbr_nexthop *pbr_sec_nexthop_old = NULL;
  struct pbr_rmap_set set_data;
#else
  ...
  if (nh_type == NEXTHOP_PRIMARY)
#endif /* HAVE_PBR */
    {
          ...
          if (rule->cmd == cmd)
            {
              ...
              if (vr->pbr_event.event_hook) 
                {
                  pal_mem_set (&set_data, 0, sizeof (struct pbr_rmap_set));
                  set_data.index = index;
                  set_data.pbr_nexthop = rule->pbr_pri_nexthop;
                  set_data.cmd = RMAP_POLICY_DELETE;

                  (*vr->pbr_event.event_hook)(PBR_RMAP_EVENT_SET_IP_NEXTHOP_ADD_DEL,
                                          vr, (void *)&set_data);
                }
              pbr_nexthop_free (rule->pbr_pri_nexthop);
              rule->pbr_pri_nexthop = NULL;
 
              if (rule->pbr_sec_nexthop)
                {
                  pbr_sec_nexthop_old = pbr_nexthop_copy (pbr_sec_nexthop_old, 
                                        rule->pbr_sec_nexthop, NEXTHOP_SECONDARY);    
                  pbr_nexthop_free (rule->pbr_sec_nexthop);
                  rule->pbr_sec_nexthop = NULL;
                }
#endif /* HAVE_PBR */
               route_map_rule_delete (&index->set_list, rule);
               replaced = 1;
      ...
      rule->flags = flags;
#ifdef HAVE_PBR
      rule->pbr_pri_nexthop = XCALLOC (MTYPE_PBR_NEXTHOP, 
                                       sizeof (struct pbr_nexthop));
      rule->pbr_pri_nexthop->nexthop_str = XSTRDUP (MTYPE_PBR_STRING, nexthop);
      rule->pbr_pri_nexthop->nh_type = NEXTHOP_PRIMARY;
      if (ifname)
        rule->pbr_pri_nexthop->ifname = XSTRDUP (MTYPE_PBR_STRING, ifname);

      if (pbr_sec_nexthop_old)
        { 
          rule->pbr_sec_nexthop = pbr_nexthop_copy (rule->pbr_sec_nexthop, 
                                                    pbr_sec_nexthop_old, 
                                                    NEXTHOP_SECONDARY);   
          pbr_nexthop_free (pbr_sec_nexthop_old);
        }
      if (vr->pbr_event.event_hook)
        {
          pal_mem_set (&set_data, 0, sizeof (struct pbr_rmap_set));
      ...
      for (rule = index->set_list.head; rule; rule = next)
        {
          ...
          if (rule->cmd == cmd)     
            {
              if (rule->pbr_pri_nexthop == NULL)
                return LIB_API_SET_ERR_CANT_SET_SECONDARY_FIRST;

              if (rule->pbr_sec_nexthop == NULL)
                {
                  rule->pbr_sec_nexthop = XCALLOC (MTYPE_PBR_NEXTHOP,
                                         sizeof (struct pbr_nexthop));
                  rule->pbr_sec_nexthop->nexthop_str = XSTRDUP (
                              MTYPE_PBR_STRING, nexthop);
                  rule->pbr_sec_nexthop->nh_type = NEXTHOP_SECONDARY;
                  if (ifname)
                    rule->pbr_sec_nexthop->ifname = XSTRDUP (MTYPE_PBR_STRING, 
                                                             ifname);
                  if (vr->pbr_event.event_hook)
                    {
                      pal_mem_set (&set_data, 0, sizeof (struct pbr_rmap_set));
                      set_data.index = index;
                      set_data.pbr_nexthop = rule->pbr_sec_nexthop;
                      set_data.cmd = RMAP_POLICY_ADD;
...
int route_map_set_unset_nexthop (struct apn_vr *vr,
                               char *name, int direct, int pref,
                               char *command, char *nexthop, s_int16_t nh_type,
                               char *ifname)
{
      ...
      if (rule->cmd == cmd )
        {
          ...
          if ((rulecmp (rule->rule_str, nexthop) == 0) || (!nexthop))
            {
              ...
              if (!nexthop)
                ...
                /* If secondary nexthop configured */ 
                if (rule->pbr_sec_nexthop != NULL)
                  return LIB_API_SET_ERR_CANT_DEL_PRIMARY;
          ...
          else if (nh_type == NEXTHOP_PRIMARY)
#endif /* HAVE_PBR */
            {
              /* Execute event hook. */
              if (vr->route_map_master.event_hook)
...
int
route_map_config_encode (struct apn_vr *vr, cfg_vect_t *cv)
{
  struct route_map *map;
  struct route_map_index *index;
  struct route_map_rule *rule;
          ...
          if (pal_strcmp (rule->cmd->str, "ip next-hop") != 0)
        ...
        else
            {
              cfg_vect_add_cmd (cv, " set %s %s", rule->cmd->str,
                               rule->rule_str ? rule->rule_str : "");
#ifdef HAVE_PBR
              if (rule->pbr_pri_nexthop && rule->pbr_pri_nexthop->ifname)
                cfg_vect_add_cmd (cv, " interface %s ", rule->pbr_pri_nexthop->ifname);
#endif /* HAVE_PBR */
              ...
              if (rule->pbr_sec_nexthop &&
                ...
                {
                  cfg_vect_add_cmd (cv, " set %s %s", rule->cmd->str,
                                    rule->pbr_sec_nexthop->nexthop_str ?
                                    rule->pbr_sec_nexthop->nexthop_str : "");
                  if (rule->pbr_sec_nexthop->ifname)
                    cfg_vect_add_cmd (cv, " interface %s", rule->pbr_sec_nexthop->ifname);
                  cfg_vect_add_cmd (cv, " secondary %s\n", 
                         CHECK_FLAG (rule->flags, ROUTE_MAP_FLAG_ADDITIVE)
                         ? " additive" : "");
                }
#endif /* HAVE_PBR */
     ...
     "IP address of next hop")
{
  ...
  ret = route_map_set_vpnv4_nexthop_set (cli->vr, index->map->name,
                                         index->type, index->pref, argv[0]);

  return lib_vty_return (cli, ret);
}

CLI (no_set_vpnv4_nexthop,
     ...
     "IP address of next hop")
{
  ...
  ret = route_map_set_vpnv4_nexthop_unset (cli->vr, index->map->name,
                                           index->type, index->pref, arg);
...
void
route_map_init_vpnv4 (struct lib_globals *zg)
{
  /* "set vpnv4 next-hop". */
  CLI_INSTALL (zg, RMAP_MODE, PM_RMAP, &set_vpnv4_nexthop_cmd);
  cli_set_imi_cmd (&route_map_cmd, RMAP_MODE, CFG_DTYP_RMAP);
  CLI_INSTALL (zg, RMAP_MODE, PM_RMAP, &no_set_vpnv4_nexthop_cmd);
  cli_set_imi_cmd (&route_map_cmd, RMAP_MODE, CFG_DTYP_RMAP);
}
#endif /* HAVE_VRF */
...
CLI (match_route_type,
     match_route_type_cmd,
     ...
     "Match OSPF External Type 2 metrics")
{
  ...
  ret = route_map_match_route_type_set (cli->vr, index->map->name,
                                        index->type, index->pref, argv[0]);

  return lib_vty_return (cli, ret);
}

CLI (no_match_route_type,
     no_match_route_type_cmd,
     ...
     "Match OSPF External Type 2 metrics")
{
  ...
  ret = route_map_match_route_type_unset (cli->vr, index->map->name,
                                          index->type, index->pref, arg);
...
CLI (set_metric_type_ospf,
     set_metric_type_ospf_cmd,
     ...
     "OSPF external type 2 metric")
{
  ...
  if (pal_strcmp (argv[0], "1") == 0)
    arg = "type-1";
  else if (pal_strcmp (argv[0], "2") == 0)
    arg = "type-2";
  else
    arg = argv[0];

  ret = route_map_set_metric_type_set (cli->vr, index->map->name,
                                       index->type, index->pref, arg);

  return lib_vty_return (cli, ret);
}

ALI (set_metric_type_ospf,
     set_metric_type_old_ospf_cmd,
     "set metric-type (1|2)",
     CLI_SET_STR,
     "Type of metric for destination routing protocol",
     "OSPF external type 1 metric",
...
CLI (no_set_metric_type_ospf,
     no_set_metric_type_ospf_cmd,
     ...
     "OSPF external type 2 metric")
{
  ...
  ret = route_map_set_metric_type_unset (cli->vr, index->map->name,
                                         index->type, index->pref, arg);

  return lib_vty_return (cli, ret);
}

ALI (no_set_metric_type_ospf,
     no_set_metric_type_old_ospf_cmd,
...
void
route_map_init_ospf (struct lib_globals *zg)
{
  /* "match tag". */
  ...
  /* "match route-type". */
  CLI_INSTALL (zg, RMAP_MODE, PM_RMAP, &match_route_type_cmd);
  cli_set_imi_cmd (&match_route_type_cmd, RMAP_MODE, CFG_DTYP_RMAP);
  CLI_INSTALL (zg, RMAP_MODE, PM_RMAP, &no_match_route_type_cmd);
  cli_set_imi_cmd (&no_match_route_type_cmd, RMAP_MODE, CFG_DTYP_RMAP);

  /* "set tag". */
  CLI_INSTALL (zg, RMAP_MODE, PM_RMAP, &set_tag_cmd);
  cli_set_imi_cmd (&set_tag_cmd, RMAP_MODE, CFG_DTYP_RMAP);
  CLI_INSTALL (zg, RMAP_MODE, PM_RMAP, &no_set_tag_cmd);
  cli_set_imi_cmd (&no_set_tag_cmd, RMAP_MODE, CFG_DTYP_RMAP);

  /* "set metric-type". */
  CLI_INSTALL (zg, RMAP_MODE, PM_RMAP, &set_metric_type_ospf_cmd);
  cli_set_imi_cmd (&set_metric_type_ospf_cmd, RMAP_MODE, CFG_DTYP_RMAP);
  CLI_INSTALL (zg, RMAP_MODE, PM_RMAP, &no_set_metric_type_ospf_cmd);
  cli_set_imi_cmd (&no_set_metric_type_ospf_cmd, RMAP_MODE, CFG_DTYP_RMAP);
  CLI_INSTALL_HIDDEN (zg, RMAP_MODE, PM_RMAP, &set_metric_type_old_ospf_cmd);
  cli_set_imi_cmd (&set_metric_type_old_ospf_cmd, RMAP_MODE, CFG_DTYP_RMAP);
  CLI_INSTALL_HIDDEN (zg, RMAP_MODE, PM_RMAP, &no_set_metric_type_old_ospf_cmd);
  cli_set_imi_cmd (&no_set_metric_type_old_ospf_cmd, RMAP_MODE, CFG_DTYP_RMAP);
}
     ...
     "Add to the existing community")
{
  ...
  for (i = 0; i < argc; i++)
  {
    if (!pal_strcmp(argv[i],"internet") || !pal_strcmp(argv[i],"local-AS") || !pal_strcmp(argv[i],"no-advertise")
        || !pal_strcmp(argv[i],"no-export") || !pal_strcmp(argv[i],"additive"))
    {
      continue;
    }
    else
...
CLI (set_metric_type_isis,
     set_metric_type_isis_cmd,
...
CLI (no_set_metric_type_isis,
     no_set_metric_type_isis_cmd,
...
CLI (set_level,
     ...
     "Export into level-2 sub-domain",
     "Export into level-1 and level-2")
{
  struct route_map_index *index = cli->index;
  int ret;

  ret = route_map_set_level_set (cli->vr, index->map->name,
                                 index->type, index->pref, argv[0]);
     ...
     "Export into level-1 and level-2")
{
  ...
  ret = route_map_set_level_unset (cli->vr, index->map->name,
                                   index->type, index->pref, arg);
...
void
route_map_init_isis (struct lib_globals *zg)
{
  /* "match tag". */
  CLI_INSTALL (zg, RMAP_MODE, PM_RMAP, &match_tag_cmd);
  cli_set_imi_cmd (&match_tag_cmd, RMAP_MODE, CFG_DTYP_RMAP);
  ...
  /* "set metric-type". */
  CLI_INSTALL (zg, RMAP_MODE, PM_RMAP, &set_metric_type_isis_cmd);
  cli_set_imi_cmd (&set_metric_type_isis_cmd, RMAP_MODE, CFG_DTYP_RMAP);

  CLI_INSTALL (zg, RMAP_MODE, PM_RMAP, &no_set_metric_type_isis_cmd);
  cli_set_imi_cmd (&no_set_metric_type_isis_cmd, RMAP_MODE, CFG_DTYP_RMAP);

  CLI_INSTALL (zg, RMAP_MODE, PM_RMAP, &set_level_cmd);
  cli_set_imi_cmd (&set_level_cmd, RMAP_MODE, CFG_DTYP_RMAP);

  CLI_INSTALL (zg, RMAP_MODE, PM_RMAP, &no_set_level_cmd);
  cli_set_imi_cmd (&no_set_level_cmd, RMAP_MODE, CFG_DTYP_RMAP);
}

void
route_map_init_rip (struct lib_globals *zg)
{
  /* "match tag". */
  CLI_INSTALL (zg, RMAP_MODE, PM_RMAP, &match_tag_cmd);
  CLI_INSTALL (zg, RMAP_MODE, PM_RMAP, &no_match_tag_cmd);
...
void
route_map_init (struct apn_vr *vr)
{
...
#ifdef HAVE_VRF
  route_map_install_set_default (vr, &route_map_vpnv4_nexthop_cmd);
#endif /* HAVE_VRF */
...
void
route_map_finish (struct apn_vr *vr)
{
...
#ifdef HAVE_VRF
  /* Install VPNv4 route map CLIs. */
  route_map_init_vpnv4 (zg);
#endif /* HAVE_VRF */

  /* Install OSPF specific route map CLIs. */
  route_map_init_ospf (zg);

  /* Install ISIS specific route map CLIs. */
  route_map_init_isis (zg);

  /* Install BGP specific route map CLIs. */
  route_map_init_bgp (zg);

  /* Install RIP specific route map CLIs. */
  route_map_init_rip (zg);
}
