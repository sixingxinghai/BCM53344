...
{
  return  hal_msg_generic_request (HAL_MSG_IGMP_SNOOPING_INIT, NULL, NULL);
}
/* 
...
{
  return  hal_msg_generic_request (HAL_MSG_IGMP_SNOOPING_DEINIT, NULL, NULL);
}

/* 
   Name: _hal_igmp_snooping 

   Description:
     Common function for enabling and disabling IGMP snooping on a bridge. 
...
static int
_hal_igmp_snooping (char *bridge_name, int msgtype)
{
  int ret;
  u_char *pnt;
  u_int32_t size;
  ...
  pnt = (u_char *) &req.msg;
  size = sizeof (struct hal_msg_igs_bridge);

  msgsz = hal_msg_encode_igs_bridge(&pnt, &size, &msg);
  if (msgsz < 0)
    return -1;
...
static int
_hal_igmp_snooping_port (char *name, unsigned int ifindex,
                         int msgtype)
{
  struct hal_nlmsghdr *nlh;
...
{ 
  return _hal_igmp_snooping (bridge_name, HAL_MSG_IGMP_SNOOPING_ENABLE);
}
...
{
  return _hal_igmp_snooping (bridge_name, HAL_MSG_IGMP_SNOOPING_DISABLE);
}

/*
  Name: _hal_igmp_snooping_entry 

  Description:
  This API adds/removes igmp learned l2 mcast entry to/from a bridge. 

  Parameters:
  IN -> bridge_name - bridge name
  IN -> src       - multicast source address
  IN -> group     - multicast group address
  IN -> is_exclude- filter mode.
  IN -> vid       - vlan id
  ...
  HAL_SUCCESS
*/
/* Common function for add/delete IGMP entry. */
static int
_hal_igmp_snooping_entry (char *bridge_name,
                          ...
                          int command)
{
  ...
  /* Set message. */
  pal_mem_set (&msg, 0, sizeof (struct hal_msg_igmp_snoop_entry));
  pal_strcpy (msg.bridge_name, bridge_name);
  msg.src.s_addr = src->s_addr;
  ...
  pnt = (u_char *) req.buf;
  size = sizeof (req.buf);
  msgsz = hal_msg_encode_igs_entry (&pnt, &size, &msg);
  if (msgsz < 0)
    return -1;
...
hal_igmp_snooping_add_entry (char *bridge_name,
                             ...
                             u_int32_t *ifindexes)
{
  return _hal_igmp_snooping_entry (bridge_name, src, group, is_exclude, vid,
                                   count, ifindexes,
                                   HAL_MSG_IGMP_SNOOPING_ENTRY_ADD);
}
...
hal_igmp_snooping_delete_entry (char *bridge_name,
                                ...
                                u_int32_t *ifindexes)
{
  return _hal_igmp_snooping_entry (bridge_name, src, group, is_exclude, vid, count, 
                                   ifindexes, HAL_MSG_IGMP_SNOOPING_ENTRY_DELETE);
}
...
{
  return  _hal_igmp_snooping_port (name, ifindex, HAL_MSG_IGMP_SNOOPING_ENABLE_PORT);
}
...
{
  return  _hal_igmp_snooping_port (name, ifindex, HAL_MSG_IGMP_SNOOPING_DISABLE_PORT);
}
