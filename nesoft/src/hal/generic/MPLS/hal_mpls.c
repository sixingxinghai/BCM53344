...
/*
  Name : hal_mpls_vpls_fib_add

  Description:
  This API adds a VPLS FIB(FTN/ILM) entry for a VPLS peer.  This API can also be used to modify an existing FTN entry.

  Parameters:
  IN -> vc_id : VC identifier
  IN -> vpls_id : VPLS Identifier
  IN -> vpls_vc_type : Type of vc (Mesh, Spoke)
  IN -> ilm_in_ifindex : Incoming interface index for incoming label
  IN -> ilm_in_label :  Incoming VPLS label
  IN -> ftn_vpls_label : VPLS label to be pushed on the outgoing packet
  IN -> ftn_vpls_peer : Address of VPLS neighbor
  IN -> ftn_vpls_nhop : Address of nexthop to reach vpls neighbor
  IN -> ftn_vpls_ifindex : Outgoing interface to reach vpls neighbor
  IN -> ftn_opcode : MPLS opcode for VPLS FTN entry
  IN -> ftn_tunnel_label : Tunnel label for carrying vpls traffic
...
*/
int
hal_mpls_vpls_fib_add (u_int32_t vc_id,
                       u_int32_t vpls_id,
                       u_char vpls_vc_type,
                       u_int32_t ilm_in_ifindex,
                       u_int32_t ilm_in_label,
                       u_int32_t ftn_vpls_label,
                       struct pal_in4_addr *ftn_vpls_peer,
                       struct pal_in4_addr *ftn_vpls_nhop,
                       u_int32_t ftn_vpls_ifindex,
                       u_char ftn_opcode,
...
/*
  Name : hal_mpls_vpls_fib_delete

  Description:
  This API deletes a VPLS FIB entry(FTN/ILM). If the identifier doesnot match, then the delete operation fails.
...
*/
int
hal_mpls_vpls_fib_delete (u_int32_t vc_id,
                          u_int32_t vpls_id,
                          u_char vpls_vc_type,
                          u_int32_t ilm_in_ifindex,
                          u_int32_t ilm_in_label,
                          struct hal_in4_addr *ftn_vpls_peer)
{
  return 0;
}
